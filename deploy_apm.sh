...
cp -f "$OLDPWD/APM-REPO-FILES/"* .

# Copy distribution package
if [ -f "$OLDPWD/dist/apm-v$VERSION.tar.gz" ]; then
    cp "$OLDPWD/dist/apm-v$VERSION.tar.gz" .
...
git push origin main || git push origin master || echo "Push failed, trying alternative approach"
