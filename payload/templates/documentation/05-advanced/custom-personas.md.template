# Creating Unified Context Engineering Personas

This guide covers creating specialized unified context engineering personas from scratch to meet unique organizational needs.

## Overview

Unified context engineering personas extend Coherence's built-in agent capabilities with:
...
```bash
{{Coherence_ROOT}}/agents/voice/speakSecurityAnalyst.sh "Security Analyst activated. Initializing security frameworks and scanning tools. Ready for threat analysis and compliance validation."
```
...
Coherence_ROOT="${Coherence_ROOT:-$HOME/.apm}"

echo "Installing Custom Persona: ${PERSONA_NAME}"

# Create persona directory structure
mkdir -p "${Coherence_ROOT}/personas/${PERSONA_NAME}"/{definition,templates,integrations,tests}

# Copy persona files
cp -r "${PERSONA_DIR}/"* "${Coherence_ROOT}/personas/${PERSONA_NAME}/"

# Generate command templates
echo "Generating command templates..."
python3 "${Coherence_ROOT}/scripts/generate-persona-templates.py" \
  --persona "${PERSONA_NAME}" \
  --config "${PERSONA_DIR}/definition/persona.json"

# Install voice script
cp "${PERSONA_DIR}/templates/voice-script.sh" \
   "${Coherence_ROOT}/agents/voice/speak${PERSONA_NAME}.sh"
chmod +x "${Coherence_ROOT}/agents/voice/speak${PERSONA_NAME}.sh"

# Register persona with Coherence
python3 "${Coherence_ROOT}/scripts/register-persona.py" \
  --name "${PERSONA_NAME}" \
  --config "${PERSONA_DIR}/definition/persona.json"

# Validate installation
echo "Validating persona installation..."
python3 "${Coherence_ROOT}/scripts/validate-persona.py" \
  --persona "${PERSONA_NAME}"
...
update_persona() {
    ...
    cp "${Coherence_ROOT}/personas/${persona_name}/definition/persona.json" \
       "${Coherence_ROOT}/personas/${persona_name}/definition/persona.json.backup"
    
    # Apply updates
    python3 "${Coherence_ROOT}/scripts/update-persona.py" \
      --persona "${persona_name}" \
      --version-check
}
...
monitor_performance() {
    ...
    python3 "${Coherence_ROOT}/scripts/collect-metrics.py" \
      --persona "${persona_name}" \
      --duration "24h" \
      --output "metrics.json"
      
    # Generate performance report
    python3 "${Coherence_ROOT}/scripts/performance-report.py" \
      --input "metrics.json" \
      --format "html"
}
```
...
- **Integration Tests**: Validate persona interactions with Coherence framework
