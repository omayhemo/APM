...
VERSION=$(cat "$PAYLOAD_DIR/VERSION" 2>/dev/null || echo "4.3.2")
...
# BUT: If a target directory was explicitly passed as $1, skip this interactive mode
if [ "$ORIGINAL_ARG_COUNT" -le 1 ] && [ -f "$PAYLOAD_DIR/install.sh" ] && [ "$1" = "." ]; then
    if [ "$USE_DEFAULTS" = true ]; then
        echo "Running with defaults - installing to user directory"
        TARGET_DIR="$HOME"
...
# Function to generate personas using simplified generation system
...
# Clean up existing .apm folder to ensure fresh installation (for ALL installation types)
...
if [ "$SKIP_COPY" != "true" ]; then
    if [ -d "$INSTALLER_DIR/templates/agents" ]; then
        ...
        # Generate personas using simplified generation system
        generate_personas_simplified
    ...
    echo "Using existing project directory - creating fresh APM structure"
    ensure_dir "$APM_ROOT"
    ...
    echo "✅ APM structure created for existing project"
fi
...
# Note: Claude Sub-Agent personas are now generated by the simplified generation system above
echo ""
...
echo "✅ Obsolete .claude/agents files cleaned up automatically"
